...
import { getProfile, updateProfile, updateAvatar, getCachedProfile, saveCachedProfile } from '@/utils/profile'
...
onMounted(async () => {
  ...
  if (info) {
    ...
    if (creator) {
      ...
      if (cachedProfile) {
        userForm.nickname = cachedProfile.nickname || userForm.nickname
        userForm.email = cachedProfile.email || ''
        userForm.gender = cachedProfile.gender ?? 0
        userForm.desc = cachedProfile.desc || ''
        userForm.avatar = cachedProfile.avatar || userForm.avatar
      }
